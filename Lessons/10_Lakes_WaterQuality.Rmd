...
> Variable 1: chlorophyll-a (chla)

> Variable 2: Secchi depth (secchi)

> Variable 3: Total organic carbon (toc)
...
LAGOSland <- LAGOSdata$buffer500m.lulc
...
combined.lagos <- 
  ...
  select(lagoslakeid,state, state, state_name, 
         gnis_name, sampledate, chla, tp, tn, secchi, 
         toc) %>%
...
LAGOSland$lagoslakeid <- as.factor(LAGOSland$lagoslakeid)

full.lagos <- 
  left_join(combined.lagos, LAGOSland, by = "lagoslakeid") %>%
  select(lagoslakeid, state, state_name, gnis_name, sampledate,
         season, chla, tp, tn, secchi, toc, buffer500m_nlcd1992_pct_81, 
         buffer500m_nlcd1992_pct_82) 

#levels(full.lagos$season) <- c("Winter", "Spring", "Summer", "Fall")

colnames(full.lagos)[colnames(full.lagos)=="buffer500m_nlcd1992_pct_81"] <- "Pasture"
colnames(full.lagos)[colnames(full.lagos)=="buffer500m_nlcd1992_pct_82"] <- "RowCrop"

full.lagos <- full.lagos %>% mutate(CropPasture = rowSums(.[11:12]))
...
corr.lagos <- full.lagos %>% select(chla, tp, tn, secchi, toc, CropPasture) %>% drop_na(chla:CropPasture)

corr.x <- cor(corr.lagos)
corrplot(corr.x, upper = "ellipse")
