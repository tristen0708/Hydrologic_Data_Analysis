...
> We would want to look at things that change over much smaller timescales - more daily, like temperature or dissolved oxygen. 
> Contaminants that could dissolve quickly in the water
> Sudden storms/flash floods
...
plot_grid(ggplot(dischargedatTFC2018, aes(x = dateTime, y = Flow_Inst)) +
    geom_line() + 
    #scale_y_log10() +
...
dygraph(cbind(Flow = with(TFC2018, xts(Flow_Inst, order.by = dateTime)), 
...
Export <- Export %>% mutate(Baseflow_perc = BaseflowExport_cf/TotalExport_cf, Quickflow_perc = QuickflowExport_cf/TotalExport_cf) 

#Baseflow Percent - 51.9%
#Quickflow Percent - 48.1%
...
dygraph(cbind(Flow = with(EC2018, xts(Flow_Inst, order.by = dateTime)), 
...
Export_EC <- Export_EC %>% mutate(Baseflow_perc = BaseflowExport_cf/TotalExport_cf, Quickflow_perc = QuickflowExport_cf/TotalExport_cf) 

#Baseflow Percent - 43.4%
#Quickflow Percent - 56.6%

```

> EC dominated more by quickflow

What does this mean physically about the watershed?

> More overland flow in EC
...
> Urban streams erode more... they are deeper (more incised) and are flashier .. aslso some water quality parameters that are related to this
...
RBI_TFC2018 <- (sum(abs(diff(dischargedatTFC2018$Flow_Inst))) / 
    sum(dischargedatTFC2018$Flow_Inst[-1])) / TFC.catchment.size

ECsitedata <- readNWISsite(site = "0208675010")
EC.catchment.size <- ECsitedata$drain_area_va #square miles

RBI_EC2018 <- (sum(abs(diff(dischargedatEC2018$Flow_Inst))) / 
    sum(dischargedatEC2018$Flow_Inst[-1])) / EC.catchment.size
...
```{r my try with Ecofina Creek}

dischargedatEF2018 <- readNWISuv(
  site = "02359500", #Ecofina Creek in Florida
  parameterCd = "00060",
  startDate = "2018-01-01", 
  endDate = "2019-01-01") %>% 
  renameNWISColumns()

EFbaseflow <- BaseflowSeparation(
  dischargedatEF2018$Flow_Inst, 
  filter_parameter = 0.925, 
  passes = 3)

EF2018 <- cbind(dischargedatEF2018, EFbaseflow)

ggplot(EF2018, aes(x = dateTime, y = Flow_Inst)) + 
  geom_line() +
  scale_y_log10() +
  geom_line(mapping = aes(x = dateTime, y = bt), color = "darkorange4") +
  geom_line(mapping = aes(x = dateTime, y = qft), color = "steelblue4")

dygraph(cbind(Flow = with(EF2018, xts(Flow_Inst, order.by = dateTime)), 
    Baseflow = with(EF2018, xts(bt, order.by = dateTime)), 
    Quickflow = with(EF2018, xts(qft, order.by = dateTime)))) %>% 
  dyRangeSelector()
...
Export_EF <- Export_EF %>% mutate(Baseflow_perc = BaseflowExport_cf/TotalExport_cf, Quickflow_perc = QuickflowExport_cf/TotalExport_cf) 


EFsitedata <- readNWISsite(site = "02359500")
EF.catchment.size <- EFsitedata$drain_area_va #square miles

RBI_EF2018 <- (sum(abs(diff(dischargedatEF2018$Flow_Inst))) / 
    sum(dischargedatEF2018$Flow_Inst[-1])) / EF.catchment.size
