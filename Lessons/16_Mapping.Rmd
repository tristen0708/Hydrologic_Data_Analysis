...
waterfeatures.NC <- filter(waterfeatures, STATE == "NC")

# Remove a couple feature types we don't care about
waterfeatures.NC <- filter(waterfeatures.NC, FEATURE != "Apparent Limit" & FEATURE != "Closure Line")
```
...
```{r}
st_crs(waterfeatures.NC)
st_crs(HUC6.NC)
waterfeatures.NC <- st_set_crs(waterfeatures.NC, 4269)
st_crs(waterfeatures.NC)
...
NClayers <- ggplot() +
  ...
  geom_sf(data = waterfeatures.NC, aes(color = FEATURE)) +
