...
> Verde River: 1915 - 2019

> Bitterroot River: 1937 - 2019

> Sauk River: 1909 - 2019

> Nehalem River: 2001 - 2019
...
#Let's take a look at the Eno River for some reason
EnoDischarge.Historical <- readNWISdv(siteNumbers = "02096500",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "1969-09-01",
                     endDate = "2019-08-31")

names(EnoDischarge.Historical)[4:5] <- c("Discharge", "Approval.Code")

EnoDischarge.Historical <- mutate(EnoDischarge.Historical, 
                                         DOY = yday(Date))

EnoDischarge.Pattern <- EnoDischarge.Historical %>%
  group_by(DOY) %>%
  summarise(Median.Discharge = median(Discharge), 
            Max.Discharge = max(Discharge),
            Min.Discharge = min(Discharge))
...
EnoPatternPlot <- 
  ggplot(EnoDischarge.Pattern, aes(x = DOY)) +
  geom_line(aes(y = Median.Discharge)) +
  geom_line(aes(y = Max.Discharge), color = "gray") +
  geom_line(aes(y = Min.Discharge), color = "gray") +  
  labs(x = "Day of Year", y = expression("Discharge (ft"^3*"/s)")) 
print(EnoPatternPlot)
